...
(select facultad,
case
when 	facultad	like 	 'Instituto Tecnológico Ferroviario Scalabrini Ortiz' then 1
when 	facultad	like 	 'Instituto del Transporte' then 1
when 	facultad	like 	 'Escuela de Ciencia y Tecnología' then 2
...
when 	facultad	like 	 'Instituto de Investigación e Ingeniería Ambiental' then 4
when 	facultad	like 	 'Instituto de Investigaciones Biotecnológicas' then 5
...
when 	facultad	like 	 'Instituto de Ciencias de la Rehabilitación' then 8
when 	facultad	like 	 'Instituto de Ciencias de la Rehabilitación y el Movimiento' then 8
when 	facultad	like 	 'Escuela de Política y Gobierno' then 9
...
when 	facultad	like 	 'Escuela de Economía y Negocios - Centro' then 12
when 	facultad	like 	 'Escuela de Economía y Negocios - San Isidro' then 12
when 	facultad	like 	 'Escuela de Economía y Negocios - San Martín' then 12
when 	facultad	like 	 'Instituto de la Calidad Industrial' then 13
when 	facultad	like 	 'Instituto de Tecnología' then 14
when 	facultad	like 	 'Instituto de Tecnología Prof. Jorge Sábato' then 14
when 	facultad	like 	 'Instituto de Tecnología Nuclear Dan Beninson' then 15
...
-- *********
-- ACTUALIZO SIGEVA en base a Mapuche

drop table if exists mapuche.dim_personas_mapuche_sigeva ;
...
else map_dw_lt_imppresupsubdependencia.imppresupdependencia_desc end as escuela,

    map_dw_lt_categoriascargo.escalafon_desc ,
    sum(ft_lt_cargos.importenetocargo) as importeneto,
    1 as cantidad 
from mapuche.ft_lt_cargos inner join mapuche.dim_personas_cargos_docentes dpc  on -- aca tengo los cuil!!, debo correr_todos_los_meses.sql
     ft_lt_cargos.persona_id = dpc.mapuche_persona_id

inner join public.persona p on 
dpc.cuil=p.cuil

inner join public.dim_periodo2 on
...
(select escuela,
case
when 	escuela	like 	 'INSTITUTO DEL TRANSPORTE'  or escuela	like 'INST TEC FERROVIARIO S ORTIZ'  then 1
when 	escuela	like 	 'ESCUELA CIENCIA Y TECNOLOGIA'  or escuela	like 'ESC. CIENCIA Y TECNOLOGIA'  then 2
when 	escuela	like 	 'ESCUELA DE HUMANIDADES' then 3
when 	escuela	like 	 'INST INVEST. E INGENIERIA AMBI' or escuela like 'INST INVEST E INGEN AMBIENTAL' then 4
when 	escuela	like 	 'INST. DE INV. BIOTECNOLOGICAS' then 5
when 	escuela	like 	 'ACTIVIDADES CENTRALES' then 6
when 	escuela	like 	 'INSTITUTO DE ARTES M. KAGEL' or escuela like 'UNIDAD DE ARTE' or escuela like 'INSTITUTO DE ARTES' then 7
when 	escuela	like 	 'INST CS REHABIL Y MOVIMIENTO' or escuela like 'INST. DE REHABILITACION' then 8
when 	escuela	like 	 'ESCUELA DE POLITICA Y GOBIERNO' then 9
when 	escuela	like 	 'INST. ALTOS ESTUDIOS SOCIALES' or escuela	like 'INSTITUTO DE  ALTOS ESTUDIOS' then 10
when 	escuela	like 	 'INST INVET PATRIMONIO CULTURAL' or escuela	like 'INST.INVESTIG.PATRIM CULTURAL' then 11
when 	escuela	like 	 'ESCUELA DE ECONOMIA Y NEGOCIOS' or escuela	like 'ESC. ECONOMIA Y NEGOCIOS' then 12
when 	escuela	like 	 'INST. DE CALIDAD INDUSTRIAL' then 13
when 	escuela	like 	 'INST. DE TEC. J. SABATO' then 14
...
-- esta es la tabla que usa el mapa1
use sigeva;
create table mapa1 as(
SELECT distinct cuil, escuela, flag, cantidad
